...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      console.log(`[PONE-ANSWER-UPLOAD] Uploading to Pone.rs: ${fileName} (${fileToUpload.size} bytes)`);
      ...
      if (poneRes.ok) {
        ...
        if (json?.success && Array.isArray(json?.files) && json.files[0]?.url) {
          ...
          console.log(`[PONE-ANSWER-UPLOAD-SUCCESS] URL: ${uploadedUrl}`);
        }
      }
    } catch (e: any) {
      console.warn("[PONE-ANSWER-UPLOAD-FAILED]", e?.message || e, "-> Switching to Catbox...");
    }
    ...
    try {
      const fileToUpload = new File([arrayBuffer], fileName, { type: mimeType });
      console.log(`[CATBOX-ANSWER-UPLOAD] Uploading to Catbox: ${fileName} (${fileToUpload.size} bytes)`);
      ...
      console.warn("[CATBOX-ANSWER-UPLOAD-FAILED]", e?.message || e, "-> Switching to Supabase Storage");
    }
